...
import { getOrDefault, defaultValue, pick, trim, md5, unique, makeDirectoryRecursively } from './index';
...
describe('pick', () => {
    ...
    test('pick one that isn\'t exist', () => {
        ...
        expect(pick<any, any>(host, ['phone'])).toMatchObject({});
    });
...
describe('unique', () => {
    ...
    test('works as expected', () => {
        ...
        expect(unique(array, value => value.replace(/[0-9]/g, ''))).toEqual(expected);
    });
...
describe('makeDirectoryRecursively', () => {
    ...
    test('makes `public/fonts` directories', async () => {
        ...
        path.resolve = jest.fn()
            .mockReturnValueOnce('/var/project/public/fonts');
    ...
    test('makes `public\\fonts` directories on Windows', async () => {
        ...
        path.sep = '\\';
        path.resolve = jest.fn()
            .mockReturnValueOnce('D:\\project\\public\\fonts');
